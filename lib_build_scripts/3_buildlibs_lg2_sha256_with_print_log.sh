...
for arch in $archs; do
    ...
    mkdir -p $build_out_tmp
    cd $build_out_tmp
    $CMAKE_PATH .. -DCMAKE_TOOLCHAIN_FILE=$toolchainfile -DCMAKE_INSTALL_PREFIX=$prefix -DCMAKE_BUILD_TYPE=Release -DUSE_SSH=ON -DBUILD_TESTS=OFF -DBUILD_CLI=OFF -DBUILD_EXAMPLES=OFF -DBUILD_FUZZERS=OFF -DBUILD_SHARED_LIBS=ON -DCMAKE_C_STANDARD=99 -DEXPERIMENTAL_SHA256=ON

    $CMAKE_PATH --build . --target install

    cp -f $prefix/lib/libgit2-experimental.so $liboutdir/libgit2-experimental.so
    # libgit2 done
